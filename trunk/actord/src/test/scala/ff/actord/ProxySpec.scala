...
/**
 * Tests the server proxy (MServerProxy) functionality.
 */
class  MServerProxySpecTest   extends JUnit3(MServerProxySpec)
object MServerProxySpecRunner extends ConsoleRunner(MServerProxySpec)
object MServerProxySpec extends Specification with MTestUtil {
  def prep = {
    // Connect to server started in a previous test (MServerWireSpec)
    // 
  ...
  "MServerProxy" should {
    ...
    "have some stats" in {
if (false) { // TODO: No Proxy stats yet.
      val (m, ea, ea2) = prep
