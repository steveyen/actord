 ...
 */
class  MServerWireSpecTest   extends JUnit3(MServerWireSpec)
object MServerWireSpecRunner extends ConsoleRunner(MServerWireSpec)
object MServerWireSpec extends Specification with MTestUtil {
  def address = InetAddress.getByName("localhost")
  def port    = 11211
  
  ff.actord.Main.main(new Array[String](0)) // Fire up a server to test.
